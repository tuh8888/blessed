...
function List(options) {
  ...
  if (options.keys) {
    this.on('keypress', function(ch, key) {
      if (key.name === 'up' || (options.vi && (key.name === 'k' || key.name === 'f'))) {
        self.up();
        self.screen.render();
        return;
      }
      if (key.name === 'down' || (options.vi && (key.name === 'j' || key.name === 's'))) {
        self.down();
        self.screen.render();
        return;
      }
      ...
      if (options.vi && (key.name === 'n' || key.ch === '-')) {
      ...
      if (options.vi && (key.ch === '/' || key.ch === '?' || key.name === 'p')) {
