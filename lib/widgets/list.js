...
function List(options) {
  ...
  if (options.keys) {
    this.on('keypress', function(ch, key) {
      if (key.name === 'up' || (options.vi && key.name === 'f')) {
        self.up();
        self.screen.render();
        return;
      }
      if (key.name === 'down' || (options.vi && key.name === 's')) {
        self.down();
        self.screen.render();
        return;
      }
      if (key.name === 'enter'
          || (options.vi && key.name === 't' && !key.shift)) {
        self.enterSelected();
        return;
      }
      ...
      if (options.vi && key.name === 'k' && key.ctrl) {
        self.move(self.height - self.iheight);
        self.screen.render();
        return;
      }
      if (options.vi && key.name === 'r' && key.shift) {
      ...
      if (options.vi && key.name === 't' && key.shift) {
